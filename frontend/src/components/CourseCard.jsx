    ...
    <Link to={`/courses/${slug}`} className="block group">
      <div className="cyber-card h-full flex flex-col group-hover:scale-[1.02] transition-transform duration-200">
        ...
        <div className="relative h-44 bg-dark-700 rounded-lg overflow-hidden mb-4">
          {thumbnail_url ? (
            ...
            <div className="w-full h-full flex items-center justify-center"
              style={{ background: `linear-gradient(135deg, ${category_color || '#8b3dff'}15, ${category_color || '#8b3dff'}05)` }}>
              <svg className="w-12 h-12 opacity-30" fill="currentColor" viewBox="0 0 20 20" style={{ color: category_color || '#8b3dff' }}>
        ...
        <div className="flex-1 flex flex-col gap-3">
          {category_name && (
            <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: category_color || '#8b3dff' }}>
              {category_name}
            </span>
          )}
